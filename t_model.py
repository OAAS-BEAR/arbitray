...
class VggSEncoder(nn.Module):
    def __init__(self):
        ...
        self.style4 = nn.Sequential()
        for i in range(4):
            self.style1.add_module("style1_" + str(i), vggsmall[i])
        for i in range(4, 11):
            self.style2.add_module("style2_" + str(i), vggsmall[i])
        for i in range(11, 18):
    ...
    def forward_c(self, x):
        ...
        style3 = self.style3(style2)
        style4 = self.style4(style3)
        style1 = self.relu(self.c1(style1))
        style2 = self.relu(self.c2(style2))
...
class styleSNet(nn.Module):
    ...
    def adaIN(self, content, style):
        c_size = content.size()
        s_size = style.size()
        content_std = (content.view(c_size[0], c_size[1], -1).var(dim=2) + 1e-6).sqrt().view(c_size[0], c_size[1], 1,
                                                                                             1).expand(c_size)
        style_std = (style.view(s_size[0], s_size[1], -1).var(dim=2) + 1e-6).sqrt().view(s_size[0], s_size[1], 1,
                                                                                         1).expand(s_size)
        content_mean = content.view(c_size[0], c_size[1], -1).mean(dim=2).view(c_size[0], c_size[1], 1, 1).expand(
            c_size)
